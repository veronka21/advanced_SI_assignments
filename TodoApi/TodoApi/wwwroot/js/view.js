class View {
     ...
     * @param {!string} name New name for the item in edit
     */
    editItemDone(id, name) {
        ...
        listItem.querySelector('label').textContent = name;
    }
    ...
    bindEditItemSave(handler) {
        View.delegate(this.$todoList, 'li .edit', 'blur', ({target}) => {
            if (!target.dataset.iscanceled) {
                let item = {
                    id: View.itemId(target),
                    name: target.value.trim(),
                    isComplete: View.itemIsComplete(target)
                };
                handler(item);
            }
        }, true);
    ...
    static itemIsComplete(element) {
        return element.parentNode.classList.contains("completed");
    }
